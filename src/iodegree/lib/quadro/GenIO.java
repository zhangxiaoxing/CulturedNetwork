...
public class GenIO {
    ...
    public GenIO(String path, boolean gluOnly) {
        pathToFile = path;
        db = new GenConnDB(pathToFile);
        r = new Well44497b();
        binSize = 50;
    ...
    private void degreeQuad(int id1, int id2, int id3, int id4, HashMap<Integer, Boolean> map, boolean isGlu, boolean isInput) {
        int count = 0;
        count += getType(id1, id2, map, isGlu, isInput);
        count += getType(id1, id3, map, isGlu, isInput);
        count += getType(id1, id4, map, isGlu, isInput);
        targetDegree[count]++;
        count = 0;
        count += getType(id2, id1, map, isGlu, isInput);
        count += getType(id2, id3, map, isGlu, isInput);
        count += getType(id2, id4, map, isGlu, isInput);
        targetDegree[count]++;
        count = 0;
        count += getType(id3, id1, map, isGlu, isInput);
        count += getType(id3, id2, map, isGlu, isInput);
        count += getType(id3, id4, map, isGlu, isInput);
        targetDegree[count]++;
        count = 0;
        count += getType(id4, id1, map, isGlu, isInput);
        count += getType(id4, id2, map, isGlu, isInput);
        count += getType(id4, id3, map, isGlu, isInput);
        targetDegree[count]++;
    }

    private void degreeTri(int id1, int id2, int id3, HashMap<Integer, Boolean> map, boolean isGlu, boolean isInput) {
        int count = 0;
        count += getType(id1, id2, map, isGlu, isInput);
        count += getType(id1, id3, map, isGlu, isInput);
        targetDegree[count]++;
    ...
    private void degreeDuo(int id1, int id2, HashMap<Integer, Boolean> map, boolean isGlu, boolean isInput) {
        targetDegree[getType(id1, id2, map, isGlu, isInput)]++;
        targetDegree[getType(id2, id1, map, isGlu, isInput)]++;
    }

    private int getType(int id1, int id2, HashMap<Integer, Boolean> map, boolean isGlu, boolean isInput) {
        int key;
        if (isInput) {
            key = getKey(id2, id1);
        } else {
        ...
        return (map.containsKey(key) && (map.get(key) == isGlu)) ? 1 : 0;
    }

    public int[] getObsDegree(boolean isGlu, boolean isInput) {
        int[] newDeg = {0, 0, 0, 0};
        obsDegree = newDeg;
        ...
        pattCount(connIdMap, isGlu, isInput);
        obsDegree[0] -= isInput ? 0 : isGlu ? gabaCells.size() : gluCells.size();
        return obsDegree;
    }
...
//        D.tp(idx);
    ...
    public int[] getExpDegree(boolean isGlu, boolean isInput) {
        int[] newDeg = {0, 0, 0, 0};
        expDegree = newDeg;
        targetDegree = expDegree;
        HashMap<Integer, Boolean> connIdMap = genConnIdMap();
        pattCount(connIdMap, isGlu, isInput);
        expDegree[0] -= isInput ? 0 : isGlu ? gabaCells.size() : gluCells.size();
        return expDegree;
    }

    public int getTotalCell(boolean isGlu, boolean isInput) {
        if (isInput) {
            return gluCells.size() + gabaCells.size();
        } else {
            return isGlu ? gluCells.size() : gabaCells.size();
        }
    }

    private void pattCount(HashMap<Integer, Boolean> connIdMap, boolean isGlu, boolean isInput) {
        //Pattern Count
        for (int currPs = 0; currPs < slots.size();) {
            ...
            if ((remains > 11)
                ...
                degreeQuad(id1, id2, id3, id4, connIdMap, isGlu, isInput);
                currPs += 12;
            } else if ((remains > 5)
                    && getKey(slots.get(currPs).getDate(), slots.get(currPs).getGroup())
                ...
                degreeTri(id1, id2, id3, connIdMap, isGlu, isInput);
                currPs += 6;
            } else {
                //duo
                int id1 = slots.get(currPs).getId1();
                int id2 = slots.get(currPs + 1).getId1();
                degreeDuo(id1, id2, connIdMap, isGlu, isInput);
                currPs += 2;
            }
        }
    }
